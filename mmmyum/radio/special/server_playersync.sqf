...
//diag_log ("playerGear: " +str(_playerGear));
	...
	if (_characterID != "0") then {		
		...
		*/
		_kills = 		["zombieKills",_character] call server_getDiff; //zeds
		_killsB = 		["banditKills",_character] call server_getDiff; //terrorist
		_killsH = 		["humanKills",_character] call server_getDiff; //resistance
		_headShots = 	["headShots",_character] call server_getDiff; 
		_humanity = 	["humanity",_character] call server_getDiff2; //good/bad
		_killsA = 		["alienKills",_character] call server_getDiff; //aliens
		_killsE = 		["esfKills",_character] call server_getDiff; //ESF
		_killsF = 		["fourthKills",_character] call server_getDiff; //4thR
		//one for type that player currently is - not a get diff
		//_humanity = 	_character getVariable ["humanity",0];
		_character addScore _kills;		//gotta do something here
		/*
